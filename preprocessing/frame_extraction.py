...
parser.add_argument('--output_root', help='path to directory')
...
for root, dirs, files in tqdm(to_iterate, total=len(to_iterate)):
    for file in files:
        if file.endswith('.mp4'):
            ...
            out_path = image_path.replace(args.input_root, args.output_root)[:-4] + '/' # Cut .mp4 suffix
            out_path = out_path.replace('videos', 'frames')
